...
namespace System.IO.Compression
{
    ...
    public class ZipStorer : IDisposable
    {
        ...
        sealed class CrcStream : Stream
        {
            ...
            /// </summary>
            public uint ReadCrc => readCrc.GetDigest();

            /// <summary>
            /// Gets the CRC checksum of the data that was written to the stream thus far.
            /// </summary>
            public uint WriteCrc => writeCrc.GetDigest();
        ...
        sealed class SegmentStream : Stream
        {
            ...
            void Invariant()
            {
                Contract.Invariant(0 <= Position);
                Contract.Invariant(Position <= Length);
            }
        }
        ...
        public bool ExtractFile(ZipFileEntry _zfe, Stream _stream)
        {
            ...
            if (_zfe.Method == Compression.LZMA)
            {
                ...
                {
                    throw new InvalidDataException("Invalid LZMA stream signature");
                }

                if (inStream.Read(buffer, 0, PropsLength) != PropsLength)
                {
                    throw new InvalidDataException("Invalid LZMA properties length");
                }
            ...
            {
                throw new InvalidDataException("Uncompressed file CRC mismatch");
            }
            return true;
        }
        /// <summary>
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            switch (_zfe.Method)
            {
                ...
                case Compression.Deflate:
                    using (var zippedStream = new DeflateStream(this.ZipFileStream, CompressionMode.Compress, true))
                    {
                        inStream.CopyTo(zippedStream);
                    }
                    break;
